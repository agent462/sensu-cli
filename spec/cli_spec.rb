...
describe 'SensuCli::Cli' do
  ...
  describe 'client commands' do
    ...
    it 'should return proper client list hash' do
      ...
      response = @cli.global
      response.should eq(:command => 'clients', :method => 'Get', :fields => { :limit => nil, :offset => nil, :format => nil, :fields => nil, :filter => nil, :help => false })
    end
    ...
    it 'should paginate with limit and offset' do
      ...
      response = @cli.global
      response.should eq(:command => 'clients', :method => 'Get', :fields => { :limit => '2', :offset => '3', :format => nil, :fields => nil, :filter => nil, :help => false, :limit_given => true, :offset_given => true })
    end

    it 'should paginate with limit' do
      ARGV.push('client', 'list', '-l', '2')
      response = @cli.global
      response.should eq(:command => 'clients', :method => 'Get', :fields => { :limit => '2', :offset => nil, :format => nil, :fields => nil, :filter => nil, :help => false, :limit_given => true })
    end
  ...
  describe 'check commands' do
    ...
    it 'should return check request hash' do
      ...
      response = @cli.global
      response.should eq(:command => 'checks', :method => 'Post', :fields => { :check => 'test_check', :subscribers => ['all', 'web'], :help => false }) # rubocop:disable WordArray
    end
  end
